  ...
  return (
    <div className="max-w-[760px] mx-auto px-4 py-8 sm:py-12">
      ...
      <div className="flex items-center gap-3 mt-4">
        ...
        <div>
          <h1 className="text-2xl font-extrabold text-ink tracking-tight dark:text-gray-100">Keamanan & Escrow</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Uangmu aman sampai pekerjaan benar-benar selesai</p>
        </div>
      </div>
          ...
          {STEPS.map((s, i) => (
            <div key={s.t} className="text-center">
              <div className="relative inline-flex">
                <div className="w-16 h-16 rounded-full bg-emerald-50 text-emerald-600 ring-4 ring-emerald-100 flex items-center justify-center bg-white relative z-10 dark:bg-slate-900">
              ...
              <h2 className="font-extrabold text-ink mt-3 text-[15px] dark:text-gray-100">{s.t}</h2>
              <p className="text-[13px] text-gray-500 leading-relaxed mt-1 dark:text-gray-400">{s.d}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="mt-8 grid sm:grid-cols-2 gap-3">
        {ITEMS.map(s => (
          <div key={s.t} className="rounded-2xl border border-gray-200/70 bg-white p-5 shadow-sm dark:bg-slate-900">
            ...
            <h2 className="font-extrabold text-ink mt-3 dark:text-gray-100">{s.t}</h2>
            <p className="text-sm text-gray-600 leading-relaxed mt-1 dark:text-gray-400">{s.d}</p>
          </div>
        ))}
      </div>
