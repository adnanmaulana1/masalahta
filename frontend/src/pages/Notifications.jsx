  ...
  if (!user) {
    return (
      <div className="max-w-[1240px] mx-auto px-4 py-16 text-center">
        <h1 className="font-extrabold text-ink text-lg">Login untuk melihat notifikasi</h1>
  ...
  return (
    <div className="max-w-[720px] mx-auto px-4 py-6 sm:py-8">
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h1 className="text-xl font-extrabold text-ink">Notifikasi</h1>
          <p className="text-sm text-gray-500">{items.filter(n => !n.read).length} belum dibaca</p>
        </div>
      ...
      <div className="rounded-2xl border border-gray-200/70 bg-white shadow-sm overflow-hidden divide-y">
        {shown.length === 0 ? (
          <div className="px-4 py-14 text-center">
            ...
            <div className="font-bold text-ink text-sm">Tidak ada notifikasi</div>
            <div className="text-xs text-gray-500 mt-1">Notifikasi order, ulasan, dan info akun muncul di sini.</div>
          </div>
        ) : shown.map(n => {
            ...
            >
              ...
              <span className="flex-1 min-w-0">
                <span className="flex items-start gap-2">
                  <span className="flex-1 text-sm font-bold text-ink leading-snug">{n.title}</span>
                  {!n.read && <span className="w-2 h-2 rounded-full bg-[#0e76f1] shrink-0 mt-1.5"></span>}
                </span>
                <span className="block text-[13px] text-gray-500 leading-relaxed mt-0.5">{n.desc}</span>
